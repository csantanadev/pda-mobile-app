...
export default function Indisciplinados() {
  ...
  async function getdados() {
    ...
    try {
      setLoading(true);
      const response = await api.get('Dashboard/indisciplinados');
    ...
    return (
      <View style={styles.itemLista}>
        ...
        <View style={{ flexDirection: 'column', alignItems: 'flex-end', justifyContent: 'space-between' }}>
          <Text style={[styles.textNome, { paddingVertical: 5 }]}>{'Total: ' + item.tot_cartao}</Text>
          <View style={{flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between'}}>
            <Text style={{ fontSize: 13, color: '#616161', marginLeft: 8 }}>{'Amarelo: ' + item.pda_fjg_amarelo}</Text>
            <Text style={{ fontSize: 13, color: '#616161', marginLeft: 8 }}>{'Azul: ' + item.pda_fjg_azul}</Text>
            <Text style={{ fontSize: 13, color: '#616161', marginLeft: 8 }}>{'Vermelho: ' + item.pda_fjg_vermelho}</Text>
          </View>
        </View>
      </View>
